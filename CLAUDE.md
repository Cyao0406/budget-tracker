...
- 最新版本 **v2.6**。v2.5 上線後使用者回報的「更新功能有問題」已具體排查完成，詳見 [CHANGELOG.md](CHANGELOG.md) 的 v2.6 條目，重點是：報表分頁改成月度／年度報告、輸入分頁進階選項的分類改直接點格子選、日曆分頁比照參考截圖重新設計、金額可輸入 0、雲端登入分類重複問題的根本修正（+ 新增「清理重複分類」「重新套用關鍵字分類」兩個一次性維護工具）、刪除分類新增「不指定分類」選項、修正刪除分類轉移選單被設定面板蓋住的疊層 bug。
- 雲端登入分類重複的修法（`handleSignedIn` 偵測「本機分類是冷啟動預設值 + 雲端已有分類」就完全採用雲端版本）**還沒有用真實帳號實測過**，只驗證過邏輯本身（模擬資料）。使用者下次用無痕視窗或新裝置登入時，麻煩留意還會不會重複，之後對話如果她回報還有問題，直接查這個函式，不用重新從頭排查。
